...
def add_food():
    ...
    with connection.cursor() as cursor:
        ...
        if result:
            ...
            if new_count == 0: # new_count가 0이면 해당 행을 삭제
                delete_sql = '''
                    DELETE FROM contain 
                    WHERE food_name = %s AND cat_name = %s AND Refri_ID = %s AND
                        input_date = %s AND exp_date = %s
                '''
                cursor.execute(delete_sql, (food_name, cat_name, refr_id, input_date_str, exp_date_str))
                message = "해당 재고가 삭제되었습니다."
            else: # new_count가 0이 아니면 해당 행을 업데이트
                update_sql = '''
                    UPDATE contain 
